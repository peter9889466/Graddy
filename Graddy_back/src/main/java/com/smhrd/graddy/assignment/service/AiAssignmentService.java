...
""",
                ...
                if (!choices.isEmpty() && choices.get(0).containsKey("message")) {
                    ...
                    if (message.containsKey("content")) {
                        ...
                        try {
                            ...
                            return parseJsonContentWithJackson(content);
                        } catch (Exception e) {
                            log.warn("Jackson JSON 파싱 실패, 간단한 파싱 시도: {}", e.getMessage());
                            try {
                                return parseJsonContent(content);
                            } catch (Exception e2) {
                                log.warn("간단한 JSON 파싱도 실패, 기본 형식으로 반환: {}", e2.getMessage());
    ...
    private Map<String, Object> createDefaultAssignmentData(StudyProject studyProject, 
        ...
        data.put("title", "AI 생성 구체적 문제 - " + (studyProject != null ? studyProject.getStudyProjectName() : "스터디"));
        data.put("description", "AI가 생성한 구체적인 실습 문제입니다. 필요에 따라 수정하여 사용하세요.");
        ...
        data.put("difficulty_level", "기본");
        data.put("additional_resources", Arrays.asList("기본 자료"));
    ...
    private String createIntegratedDescription(Map<String, Object> aiAssignmentData) {
        ...
        if (aiAssignmentData.containsKey("learning_objectives") && aiAssignmentData.get("learning_objectives") != null) {
        ...
        if (aiAssignmentData.containsKey("evaluation_criteria") && aiAssignmentData.get("evaluation_criteria") != null) {
        ...
        if (aiAssignmentData.containsKey("additional_resources") && aiAssignmentData.get("additional_resources") != null) {
