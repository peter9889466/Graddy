...
const Schedule: React.FC<ScheduleProps> = ({
    ...
    const [activeTab, setActiveTab] = useState<"assignment" | "schedule">(
        "assignment"
    );
    ...
    const [aiGeneratedAssignment, setAiGeneratedAssignment] =
        useState<any>(null);
    ...
    useEffect(() => {
        console.log(
            "Schedule 컴포넌트 마운트 - studyProjectId:",
            studyProjectId
        );
        if (studyProjectId > 0) {
            loadAssignments();
    ...
    const loadAssignments = async () => {
        ...
        try {
            console.log(
                "과제 목록 로드 시도 - studyProjectId:",
                studyProjectId
            );

            // DB에서 과제 목록 가져오기
            const response = await apiGet(
                `/assignments/study-project/${studyProjectId}`
            );
    ...
    const loadSchedules = async () => {
        try {
            console.log(
                "일정 목록 로드 시도 - studyProjectId:",
                studyProjectId
            );

            // DB에서 일정 목록 가져오기
            const response = await apiGet(
                `/schedules/study-project/${studyProjectId}`
            );
            ...
            if (response.data && Array.isArray(response.data)) {
                const schedules = response.data.map((schedule: any) => ({
                    ...
                    date: new Date(schedule.schTime)
                        .toISOString()
                        .split("T")[0],
                    time: new Date(schedule.schTime)
                        .toISOString()
                        .split("T")[1]
                        .substring(0, 5),
                ...
                setScheduleItems((prevItems) => {
                    const existingAssignments = prevItems.filter(
            ...
            try {
                if (activeTab === "assignment") {
                    ...
                    const scheduleData = {
                        ...
                        schTime: new Date(
                            `${newItem.date}T${newItem.time || "00:00"}`
                        ).toISOString(),
                    };
        ...
        try {
            ...
            if (response.data) {
                ...
                setNewItem({
                    ...
                    date: new Date((response.data as any).deadline)
                        .toISOString()
                        .split("T")[0],
                    time: "",
                });
                    ...
                    <div className="space-y-3">
                        {filteredItems.map((item) => {
                            ...
                            return (
                                <div
                                    key={item.id}
                                    className={`bg-white border rounded-lg hover:shadow-md transition-shadow duration-200 ${
                                        isOverdue
