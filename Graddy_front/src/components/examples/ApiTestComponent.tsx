...
const ApiTestComponent: React.FC = () => {
    ...
    const testApiConnection = async () => {
        ...
        try {
            ...
            setTestResult(`상태 코드: ${response.status}\n상태 텍스트: ${response.statusText}\n응답: ${await response.text()}`);
        } catch (error) {
            setTestResult(`오류: ${error instanceof Error ? error.message : String(error)}`);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const testWithAuth = async () => {
        ...
        try {
            ...
            setTestResult(`인증 포함 - 상태 코드: ${response.status}\n상태 텍스트: ${response.statusText}\n응답: ${await response.text()}`);
        } catch (error) {
            setTestResult(`인증 포함 - 오류: ${error instanceof Error ? error.message : String(error)}`);
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md">
            ...
            <div className="mb-6">
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                    <p><strong>로그인 상태:</strong> {authContext?.isLoggedIn ? '로그인됨' : '로그아웃됨'}</p>
                    <p><strong>사용자:</strong> {authContext?.user?.nickname || '없음'}</p>
                    <p><strong>토큰:</strong> {authContext?.token ? '있음' : '없음'}</p>
                    <p><strong>localStorage userToken:</strong> {localStorage.getItem('userToken') ? '있음' : '없음'}</p>
                </div>
            </div>
            ...
            {testResult && (
                <div className="mb-6">
                    <h3 className="text-lg font-semibold mb-3 text-gray-700">테스트 결과</h3>
                    <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <div className="text-sm text-gray-600">
                ...
                <ul className="list-disc list-inside space-y-1">
                    <li>404 오류: 백엔드 서버가 실행되지 않았거나 엔드포인트가 잘못됨</li>
                    <li>401 오류: 인증 토큰이 없거나 유효하지 않음</li>
                    <li>CORS 오류: 브라우저에서 다른 도메인으로의 요청이 차단됨</li>
