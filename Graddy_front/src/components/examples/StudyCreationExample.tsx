...
const StudyCreationExample: React.FC = () => {
    ...
    const handleCreateStudyProject = async () => {
        ...
        try {
            ...
            const studyProjectData: CreateStudyProjectRequest = {
                ...
                studyProjectDesc: "React와 TypeScript를 함께 배우는 기초 스터디입니다. 컴포넌트 기반 개발과 타입 안정성을 중점적으로 학습합니다.",
                studyLevel: 2, // 중급
                ...
                soltStart: new Date(now.getFullYear(), now.getMonth(), now.getDate(), 19, 0, 0).toISOString(), // 오후 7시
                soltEnd: new Date(now.getFullYear(), now.getMonth(), now.getDate(), 21, 0, 0).toISOString(), // 오후 9시
    ...
    return (
        <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md">
            ...
            <div className="mb-6">
                <h3 className="text-lg font-semibold mb-3 text-gray-700">현재 로그인 상태</h3>
                <div className="bg-gray-50 p-4 rounded-lg">
                    {authContext?.user ? (
                        <div>
                            <p><strong>닉네임:</strong> {authContext.user.nickname}</p>
                            <p><strong>이메일:</strong> {authContext.user.email}</p>
                            <p><strong>토큰:</strong> {authContext.token ? '있음' : '없음'}</p>
                        </div>
                    ) : (
            ...
            <div className="mb-6">
                <h3 className="text-lg font-semibold mb-3 text-gray-700">API 요청 예시</h3>
                <div className="bg-gray-50 p-4 rounded-lg">
...
{`{
  ...
  "dayIds": ["2", "4", "6"]
            ...
            {error && (
                <div className="mb-6">
                    <h3 className="text-lg font-semibold mb-3 text-red-600">오류</h3>
            ...
            {result && (
                <div className="mb-6">
                    <h3 className="text-lg font-semibold mb-3 text-green-600">성공</h3>
            ...
            <div className="text-sm text-gray-600">
                ...
                <ul className="list-disc list-inside space-y-1">
                    <li>JWT 토큰은 자동으로 Authorization 헤더에 포함됩니다.</li>
                    <li>userId는 JWT 토큰에서 자동으로 추출되므로 선택적으로 전송할 수 있습니다.</li>
                    <li>날짜는 ISO 8601 형식으로 전송해야 합니다.</li>
                    <li>interestIds는 관심사 ID 배열입니다.</li>
                    <li>dayIds는 요일 ID 배열입니다 (1: 월요일 ~ 7: 일요일).</li>
                </ul>
            </div>
        </div>
    );
