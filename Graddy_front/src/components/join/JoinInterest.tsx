        ...
        const fetchInterests = async () => {
            ...
            try {
                ...
                if (response.data.data !== null) {
                    ...
                    const mappedInterests: InterestItem[] = result.map(
                        (item: ApiInterestItem) => ({
                            id: item.interestId,
                            name: item.interestName,
                            category:
                                categoryMapping[item.interestDivision] ||
                                "other", // 매핑되지 않으면 'other'로 설정
                        })
                    );
                ...
                setError(
                    "관심사 목록을 불러오는 데 실패했습니다. 서버가 실행 중인지 확인해주세요."
                );
            } finally {
                setIsLoading(false);
            }
